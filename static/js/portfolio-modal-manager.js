...
if (typeof window.portfolioModalManagerLoaded === 'undefined') {
  ...
  document.addEventListener('DOMContentLoaded', function() {
    ...
    window.openPortfolioModal = function(itemId) {
      ...
      // إيقاف تشغيل أي فيديو حالي قبل فتح نافذة جديدة
      ...
      // إيقاف الفيديو الخارجي (يوتيوب/فيميو) إن وجد
      const externalVideo = document.getElementById('modal-external-video');
      if (externalVideo) {
        externalVideo.src = '';
      }
      
      // إخفاء حاويات الفيديو إذا كانت ظاهرة
      const videoContainer = document.getElementById('modal-video-container');
      if (videoContainer) {
        videoContainer.style.display = 'none';
      }
      
      // إظهار حاوية الصورة إذا كانت مخفية
    ...
    // مستمع لمفتاح ESC لإغلاق النافذة المنبثقة
