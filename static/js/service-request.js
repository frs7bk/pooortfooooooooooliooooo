...
$(document).ready(function() {
    ...
    if ($('.message-container').length === 0) {
        $('#service-request-form').before('<div class="message-container my-4"></div>');
    }
    ...
    $('#service-request-form').on('submit', function(e) {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                // إظهار رسالة النجاح
                showMessage('success', response.message || 'تم إرسال طلبك بنجاح! سنقوم بالتواصل معك قريباً لمناقشة التفاصيل.');
    ...
    function showMessage(type, message) {
        // إنشاء رسالة بتنسيق مخصص محسن (بدون استخدام قوالب النصوص)
        const bgColor = type === 'success' ? 'green-500' : 'red-500';
        const iconName = type === 'success' ? 'check-circle' : 'exclamation-circle';
        
        const messageHtml = 
            '<div class="bg-' + bgColor + ' text-white p-4 rounded-lg shadow-md service-message">' +
            '<div class="flex items-center justify-between">' +
            '<div class="flex items-center">' +
            '<i class="fas fa-' + iconName + ' mr-2"></i>' +
            '<div class="font-medium">' + message + '</div>' +
            '</div>' +
            '<button type="button" class="text-white hover:text-gray-200 close-message">' +
        ...
        // تفعيل زر إغلاق الرسالة
        $('.close-message').on('click', function() {
            $(this).closest('.service-message').fadeOut(300, function() {
                $(this).remove();
            });
        });
        ...
        setTimeout(function() {
            $('.service-message').fadeOut(500, function() {
                $(this).remove();
            });
        }, 8000);
    }
