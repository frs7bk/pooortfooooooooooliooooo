...
function displayPortfolioModal(data) {
    ...
    const hasVideo = data.video_type && (data.video_file || data.video_url);
    ...
    // إضافة الفيديو إذا كان متاحًا
    let videoHTML = '';
    if (hasVideo) {
        videoHTML = createVideoSection(data);
    }
    ...
    const contentHTML = `
        ...
        ${hasVideo ? `
        <div class="modal-video-section my-3">
            <button class="btn btn-success btn-lg show-video-btn" type="button">
    ...
    // إذا كان هناك فيديو، أضف حاوية الفيديو بعد حاوية الصورة
    if (hasVideo) {
        // إضافة حاوية الفيديو بعد حاوية الصورة
        const videoContainer = document.createElement('div');
        videoContainer.className = 'modal-video-container';
        videoContainer.style.display = 'none';
        videoContainer.innerHTML = videoHTML;
        
        modalImageContainer.parentNode.insertBefore(videoContainer, modalImageContainer.nextSibling);
    }
    ...
    // إعداد تفاعل الفيديو إذا كان متاحاً
    if (hasVideo) {
        setupVideoInteractions(data);
    }
}
 ...
 * إنشاء قسم الفيديو
 * @param {Object} data - بيانات المشروع
 * @returns {string} HTML لقسم الفيديو
 */
function createVideoSection(data) {
    if (data.video_type === 'external') {
        // تحويل روابط يوتيوب إلى صيغة التضمين
        ...
        return `
            <div class="video-wrapper">
                <iframe src="${videoSrc}" frameborder="0" allowfullscreen class="embedded-video"></iframe>
            </div>
            <button class="btn btn-sm btn-light back-to-image-btn mt-2">
...
/**
 * إعداد تفاعلات الفيديو
 * @param {Object} data - بيانات المشروع
 */
function setupVideoInteractions(data) {
    // زر عرض الفيديو
    ...
    if (showVideoBtn && modalImageContainer && modalVideoContainer) {
    ...
    // زر العودة للصورة
    const backToImageBtn = document.querySelector('.back-to-image-btn');
    if (backToImageBtn && modalImageContainer && modalVideoContainer) {
